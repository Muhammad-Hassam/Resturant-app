...
const handleupdate=(data)=>{
  database.ref('/Food').child('Orders'+"/"+data.userid+"/"+data.key).update({
    status: 'Proceed to Rider',
  })
}

const alldata=allOrders.filter((data)=>data.status==="Proceed to kitchen"&&data.status!=="Proceed to Rider"&&data.status!=="Delivered"&&data.status!=="Not Delivered")
    return (
        <>
        <Head/>
        <View style={styles.container}>
            <ScrollView>
        <View>
          <Text style={theme===false?styles.breakfast:styles.breakfasttheme}>Kitchen Orders</Text>
